    ...
    return (
        <header className="fixed left-0 right-0 z-50 flex justify-center px-4 top-6">
            <div className="bg-white/70 backdrop-blur-md rounded-full border border-white/50 px-4 sm:px-8 py-3 flex items-center justify-between w-full max-w-7xl shadow-[0_4px_20px_-10px_rgba(0,0,0,0.05)]">
                {/* Logo */}
                <Link to="/" className="mr-4 sm:mr-12 font-serif text-lg sm:text-2xl font-bold tracking-tight text-gray-900 hover:text-gray-600 transition">
                    LuxeScents
                </Link>
            ...
            {/* Mobile Menu */}
            {mobileMenuOpen && (
                <div className="absolute top-20 left-4 right-4 bg-white/95 backdrop-blur-md rounded-2xl border border-white/50 shadow-lg md:hidden z-40">
                        </header>
                    <nav className="flex flex-col p-6 space-y-4">
                        <Link to="/" onClick={() => setMobileMenuOpen(false)} className="text-sm font-medium text-gray-900 hover:text-gray-600 transition">Home</Link>
                        <Link to="/shop" onClick={() => setMobileMenuOpen(false)} className="text-sm font-medium text-gray-900 hover:text-gray-600 transition">Shop</Link>
                        <Link to="/solutions" onClick={() => setMobileMenuOpen(false)} className="text-sm font-medium text-gray-900 hover:text-gray-600 transition">Solutions</Link>
                        <Link to="/about" onClick={() => setMobileMenuOpen(false)} className="text-sm font-medium text-gray-900 hover:text-gray-600 transition">About</Link>
                        <Link to="/blog" onClick={() => setMobileMenuOpen(false)} className="text-sm font-medium text-gray-900 hover:text-gray-600 transition">Blog</Link>
                        <Link to="/faq" onClick={() => setMobileMenuOpen(false)} className="text-sm font-medium text-gray-900 hover:text-gray-600 transition">FAQ</Link>
                        <Link to="/press" onClick={() => setMobileMenuOpen(false)} className="text-sm font-medium text-gray-900 hover:text-gray-600 transition">Press</Link>
                        <Link to="/support" onClick={() => setMobileMenuOpen(false)} className="text-sm font-medium text-gray-900 hover:text-gray-600 transition">Support</Link>
                        <Link to="/profile" onClick={() => setMobileMenuOpen(false)} className="text-sm font-medium text-gray-900 hover:text-gray-600 transition">Profile</Link>
                    </nav>
                </div>
            )}
        </header>
